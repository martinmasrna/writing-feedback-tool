...
**Caret movement across markup is ours to drive, not the browser's.** Struck text and reason marks are `contenteditable="false"`, and Chrome will neither put the caret inside them *nor step over them* — an arrow press beside a deletion does nothing at all, and the next keystroke lands wherever the caret was stuck. `stepCaret()` in `edits.js` moves it: plain text one character at a time, a run of finished markup skipped whole in a single press. Any new unedittable region must be reachable past, or it becomes a wall — and must have somewhere to stand on the far side of it, which is what the landing spots in `render-rendered.js` are. Without one, deleting a word off the end of a paragraph and typing the replacement put it in the paragraph below.
