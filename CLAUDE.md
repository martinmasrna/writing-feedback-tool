...
**Never interrupt editing to ask for a reason.** Reasons are a review-time act, not a typing-time one — people press Enter for room, write, delete half of it, come back ten minutes later. An earlier build prompted whenever the cursor left an edit; it re-armed itself on the next cursor move and became inescapable. Unexplained edits are *marked* — inline and in the sidebar — and explained when the writing is done, from the sidebar entry.
...
**Every way of adding or changing tracked text goes through `applyAction`, `structure.js` or `annotate()`.** Those three are where the guards live: an island must not be swallowed, an annotation must not nest inside another. Anything that reaches `edits.js` directly to create or extend an annotation bypasses them, and that has now happened three times — the floating toolbar calling `edits.annotate`, IME composition calling `edits.insert`, and `input.js` not naming an `inputType` at all. If you are adding a fourth way to change the text, it goes through one of the three.

`app.js` also calls `edits.removeAnnotation` and `edits.setReason` directly, and that's fine: neither can nest or swallow anything, since both only ever replace an annotation's own span (with the original text, or with a rewritten reason) rather than introduce a new one.
...
**A range still inside an insertion nobody has finished making is not a deletion — it is you reshaping your own draft.** `deleteBackward` already knows this for one character at a time: erasing text you just typed shrinks the `{++…++}` in place rather than tracking a strike-through against it, because there is nothing to track — the text never existed before this session. `deleteRange` — word delete, line delete, and the range Backspace computes to join two blocks back into one — did not, for years, because it was written to ask one question: does *any* annotation overlap this range? An insertion *is* an annotation, so the answer was always yes, and it refused, exactly as if the range were about to nest a new change inside someone else's finished edit. It refused a marker Enter had just written into an insertion the same way, for the same reason, and Tab inherited the same blind spot for the same range-shaped reason: its own "extend this insertion instead of nesting" case only fired when the insertion's body was *exactly* the marker, which stopped being true the moment real content followed it. `openBody()` in `criticmarkup.js` is the range version of the question `regionAt()` already answers for a point — is `[start,end)` entirely inside one still-open insertion or substitution body — and `deleteRange()` and `changeIndent()` both ask it before refusing anything, the same way `deleteBackward` already asks for a single character. Split a fresh bullet in two with Enter, then Backspace the join or Tab the second half, and both now do exactly what typing there would.
...
- Multi-file management, git integration, sync, collaboration, mobile, any backend or account. The file panel (`src/ui/files-panel.js`, served by `/roots` and `/list` in `tools/serve.js`) is a way to reach one document at a time inside the safelisted folders; it exists only under the dev server and manages nothing.

## Session close

Every session ends per `../../ops/PROTOCOL.md`: rewrite this project's section in `../../STATUS.md` to the current state, and file a queue item when Martin's judgment is the blocker.
